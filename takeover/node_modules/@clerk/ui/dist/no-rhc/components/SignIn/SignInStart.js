...
import { SIGN_IN_RESET_PASSWORD_INTENT_PARAM, hasMultipleEnterpriseConnections, useHandleAuthenticateWithPasskey } from "./shared.js";
...
function SignInStartInternal() {
	...
	*/
	...
	const signInWithFields = async (fields, options) => {
			...
			switch (res.status) {
				...
				case "needs_first_factor":
					if (!hasOnlyEnterpriseSSOFirstFactors(res) || hasMultipleEnterpriseConnections(res.supportedFirstFactors)) {
						if (options?.resetPasswordIntent) return navigate("factor-one", { searchParams: new URLSearchParams({ [SIGN_IN_RESET_PASSWORD_INTENT_PARAM]: "true" }) });
	...
	const attemptToRecoverFromSignInError = async (e) => {
		if (!e.errors) return;
		const instantPasswordError = e.errors.find((e) => e.code === ERROR_CODES.INVALID_STRATEGY_FOR_USER || e.code === ERROR_CODES.FORM_PASSWORD_INCORRECT || e.code === ERROR_CODES.FORM_PASSWORD_PWNED);
		const sessionAlreadyExistsError = e.errors.find((e) => e.code === ERROR_CODES.SESSION_EXISTS);
		const alreadySignedInError = e.errors.find((e) => e.code === "identifier_already_signed_in");
		const accountDoesNotExistError = e.errors.find((e) => e.code === ERROR_CODES.INVITATION_ACCOUNT_NOT_EXISTS || e.code === ERROR_CODES.FORM_IDENTIFIER_NOT_FOUND);
	...
	const handleForgotPasswordClick = (e) => {
		...
		signInWithFields([identifierField], { resetPasswordIntent: true });
	};
...
const InstantPasswordRow = ({ field, onForgotPasswordClick }) => {
	...
	useLayoutEffect(() => {
		const intervalId = setInterval(() => {
			if (ref?.current) {
				const autofilled = window.getComputedStyle(ref.current, ":autofill").animationName === "onAutoFillStart" || !!ref.current?.matches("*:-webkit-autofill");
				if (autofilled) {
	...
	return /* @__PURE__ */ jsx(Form.ControlRow, {
		...
		children: /* @__PURE__ */ jsx(Form.PasswordInput, {
			...field.props,
			actionLabel: show ? localizationKeys("formFieldAction__forgotPassword") : void 0,
			onActionClicked: show ? onForgotPasswordClick : void 0,
