...
const PROTECT_CHECK_SCRIPT_TIMEOUT_MS = 6e4;
/**
	...
	React.useEffect(() => {
		...
		if (protectCheck.expiresAt !== void 0 && protectCheck.expiresAt < Date.now()) {
			...
			(async () => {
				try {
					...
					if (mountedRef.current) handleError(err, [], card.setError);
				} finally {
		...
		const runChallenge = async () => {
			...
			try {
				...
				const proofToken = await Promise.race([executeProtectCheck(protectCheck, container, {
					signal: abortController.signal,
					setWidgetVisible
				}), new Promise((_, reject) => {
					timeoutId = setTimeout(() => {
						abortController.abort();
						reject(new ClerkRuntimeError("Protect verification timed out", { code: ERROR_CODES.PROTECT_CHECK_TIMED_OUT }));
